...
use crate::{App, ui::{theme, toolbar::matches_query}};
...
pub fn show(ui: &mut egui::Ui, app: &mut App) {
    ...
    egui::Frame::new()
        ...
        .show(ui, |ui| {
            egui::Grid::new("hdr").num_columns(3).min_col_width(col_del).show(ui, |ui| {
    ...
    ScrollArea::vertical().auto_shrink([false; 2]).show(ui, |ui| {
        let indices: Vec<usize> = app.store.flags.iter().enumerate()
            .filter(|(_, f)| matches_query(f, &app.search))
            .map(|(i, _)| i)
            .collect();
        ...
        egui::Grid::new("flags")
            ...
            .show(ui, |ui| {
                for idx in indices {
                    ...
                    ui.add(
                        egui::Label::new(
                            RichText::new(&flag.key).monospace().color(key_color).size(12.5),
                        )
                        .wrap_mode(egui::TextWrapMode::Extend),
                    );

                    // Value — stable Id prevents focus loss on every keystroke
                    let resp = ui.add(
                        egui::TextEdit::singleline(&mut flag.value)
                            .id(Id::new("fv").with(&flag.key))
                    ...
                    // Delete
                    if ui.add(
                        egui::Button::new(RichText::new("✕").color(theme::ACCENT_RED).size(11.0))
